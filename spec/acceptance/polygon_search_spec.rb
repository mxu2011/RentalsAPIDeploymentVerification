...
describe ' polygonsearch' do
  ...
  it "production polygon search having 500 error "  do
  ...
  it "All production polygon searches"  do

    count = 0
    err_count = 0
    File.open("#{Rails.root}/spec/fixtures/polygon_prod_all.json").each_line do |line|
      begin
        url= SERVICE_URL + JSON.parse(line)["result"]["http_request"].scan( /GET \/v1\/.+points.+HTTP/)[0].gsub("GET \/v1",'').gsub("HTTP",'')
        resp = HTTParty.get(url)
